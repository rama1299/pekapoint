  ...
  onMount(async () => {
    ...
    async function handleSelect(title) {
        try {
            ...
            const isDataExists = data.some(existingData => existingData.slug === slug);
            if (!isDataExists) {
                const feature_image = await fethcImage(slug)
                const addData = {title, slug, feature_image}
                data = [...data, addData]
                searchValue = ''
                optionTitle = []
    ...
    <div class="{isToggleBtn ? '' : 'hidden'} w-60 md:w-64">
        ...
        <div class="h-12 bg-white flex justify-center items-center w-full">
            <div class="{data.length === 3 ? 'w-full' : 'w-2/3'} h-full flex justify-center items-center">
                <button class="bg-sekunder-950 hover:bg-white hover:text-sekunder-950 rounded-full border border-sekunder-950 text-white h-8 px-2 text-sm font-medium" on:click={handleCompare}>{text[0]}</button>
            </div>
