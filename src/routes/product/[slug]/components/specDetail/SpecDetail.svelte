    ...
    <div class="container lg:w-wrap mx-auto py-10 flex justify-center items-start gap-3 lg:gap-5 bg-white px-5 lg:px-10 border">
            <div class="w-1/6 lg:w-1/4 space-y-2 py-1 sticky top-16 lg:top-24">
                {#each datas as item, i (i)}
                     <div class="bg-white cursor-pointer hover:scale-105 w-full h-[75px] lg:h-20 rounded-md flex justify-center items-center text-md font-medium border-2 hover:border-blue-600" key={i} on:click={() => {handleShowIndex(i)}}>
                        <p>{item.title}</p>
                     </div>
                {/each}
            </div>
