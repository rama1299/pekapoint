<script>
    ...
    $: key = datas.filter(data => data.title !== 'Miscellaneous').map(data => data.title);
...
<div class="w-full flex h-auto gap-5">
    <div class="w-2/12 md:w-1/12 h-screen sticky top-16 lg:top-20" bind:offsetHeight={heightContainerLagend} bind:offsetWidth={widthContainerLagend}>
    ...
    <div class="w-10/12 md:w-11/12 space-y-10">
        {#each key as title}
